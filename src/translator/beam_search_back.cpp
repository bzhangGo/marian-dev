...
Beams BeamSearch::purgeBeams(const Beams& beams, /*in/out=*/std::vector<IndexType>& batchIdxMap) {
  ...
  for(auto beam : beams) {
    ...
    for(auto hyp : beam)
      // IBDecoder: the previous word is also not EOS
      if(hyp->getWord() != trgEosId && hyp->getPrevHyp()->getWord() != trgEosId) // if this hyp is not finished,
...
Histories BeamSearch::search(Ptr<ExpressionGraph> graph, Ptr<data::CorpusBatch> batch) {
  ...
  ABORT_IF(numFactorGroups > 1, "IBDecoder Only support non-factorted translation!");
  ...
  for (size_t t = 0; ; t++) {
    ...
    for (size_t factorGroup = 0; factorGroup < numFactorGroups; factorGroup++) {
        ...
        for(size_t beamHypIdx = 0; beamHypIdx < maxBeamSize; ++beamHypIdx) { // loop over globally maximal beam-size (maxBeamSize)
          for(int origBatchIdx = 0; origBatchIdx < origDimBatch; ++origBatchIdx) { // loop over all batch entries (active and inactive)
            ...
            if(beamHypIdx < beam.size()) {
              ...
              auto prevHyp = hyp->getPrevHyp();   // IBDecoder: get previous hypothesis
              ...
              // IBDecoder: we should use the previous hypothesis for index selection
              ...
              prevWords .push_back(prevHyp->getWord()); // IBDecoder: add another previous word
              ...
              if(!PURGE_BATCH || !beam.empty()) { // but only if we are not pruning and the beam is not deactivated yet
                ...
                prevWords.push_back(trgEosId);    // IBDecoder: two words each time
      ...
      Expr currWordScores;    // [currentDimBatch, 1, actualBeamSize, topN]
      std::vector<IndexType> secondWordBeamMapper, reverseBatchIdxMap; // empty if not purging batch entries
      for(size_t wordIdx = 0; wordIdx < 2; ++ wordIdx) {
        currWordScores = index_select(nBestLocalScoresTensor, 1, {(IndexType)wordIdx});  // [currentDimBatch, 1, actualBeamSize, topN]
        
        // extract selected elements from the first word for the second word
        if(wordIdx == 1) {
          currWordScores = swapAxes(currWordScores, 0, 2);
          currWordScores = reshape(currWordScores, {(int)(currWordScores->shape().elements()/topN), (int)topN});
          currWordScores = index_select(currWordScores, 0, secondWordBeamMapper);
          currWordScores = reshape(currWordScores, {(int)actualBeamSize, 1, (int)currentDimBatch, (int)topN});
          currWordScores = swapAxes(currWordScores, 0, 2);
        ...
        for(size_t i = 0; i < nBestKeys.size(); ++i) { // [currentDimBatch, maxBeamSize] flattened
          ...
          auto actualBeamHypIdx      = beamHypIdx;
          if(wordIdx == 1) {
              const auto origBatchIdx    = reverseBatchIdxMap.empty() ? currentBatchIdx : reverseBatchIdxMap[currentBatchIdx];
              auto& beam = beams[origBatchIdx];
              actualBeamHypIdx = beam[beamHypIdx]->getPrevStateIndex();
          }
          ...
          size_t keyMapIdx = (2*currentBatchIdx + wordIdx) * (actualBeamSize * topN) + actualBeamHypIdx * topN + predIdx;
          if(wordIdx == 1 && t == 0)
        ...
        for(size_t beamHypIdx = 0; beamHypIdx < maxBeamSize; ++beamHypIdx) { // loop over globally maximal beam-size (maxBeamSize)
          for(int origBatchIdx = 0; origBatchIdx < origDimBatch; ++origBatchIdx) { // loop over all batch entries (active and inactive)
            ...
            if(beamHypIdx < beam.size()) {
              ...
              secondWordBeamMapper.push_back(hypIndex);
              prevScores.push_back(canExpand ? hyp->getPathScore() : INVALID_PATH_SCORE);
            } else {  // pad to maxBeamSize (dummy hypothesis)
              if(!PURGE_BATCH || !beam.empty()) { // but only if we are not pruning and the beam is not deactivated yet
                secondWordBeamMapper.push_back(0);
                prevScores.push_back((float)INVALID_PATH_SCORE);
              }
            }
         }
